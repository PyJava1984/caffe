...
template<typename Dtype>
void DataTransformer<Dtype>::MultiTransforms(const cv::Mat& cv_img,
                                             Blob<Dtype>* transformed_blob) {
  const int channels = transformed_blob->channels();
  const int height = transformed_blob->height();
  const int width = transformed_blob->width();
  ...
  if (phase_ != TRAIN) {
    Blob<Dtype> uni_blob(1, channels, height, width);
    int offset = transformed_blob->offset(0);
    uni_blob.set_cpu_data(transformed_blob->mutable_cpu_data() + offset);
    ...
    for (int i = 0; i < mirrors.size(); ++i) {
      ...
      for (int j = 0; j < scale_sizes.size(); ++j) {
        Blob<Dtype> uni_blob(1, channels, height, width);
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImpl(const cv::Mat& cv_img,
                                           Blob<Dtype>* transformed_blob,
                                           int crop_size,
                                           bool do_mirror,
                                           bool random_crop,
    ...
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < img_channels; ++c) {
        ...
        if (has_mean_file) {
          int mean_index = (c * img_height + h_off + h) * img_width + w_off + w;
          transformed_data[top_index] =
              (pixel - mean[mean_index]);
        } else {
          if (has_mean_values) {
            transformed_data[top_index] =
                (pixel - mean_values_[c]);
          } else {
